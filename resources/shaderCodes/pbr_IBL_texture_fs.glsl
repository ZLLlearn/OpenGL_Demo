...
vec3 getNormalFromMap() {
    vec3 tangentNormal = texture(normalMap, pTexCoord).xyz * 2.0 - 1.0;

    vec3 Q1 = dFdx(worldPos);
    vec3 Q2 = dFdy(worldPos);
    vec2 st1 = dFdx(pTexCoord);
    vec2 st2 = dFdy(pTexCoord);
...
void main() {
    vec3 albedo = pow(texture(albedoMap, pTexCoord).rgb, vec3(2.2));
    float metallic = texture(metallicMap, pTexCoord).r;
